...
uint32_t dull_red = Adafruit_NeoPixel::Color(5, 0, 0);
uint32_t bright_red = Adafruit_NeoPixel::Color(200, 0, 0);
uint32_t first_color = Adafruit_NeoPixel::Color(32, 0, 0);
uint32_t second_color = Adafruit_NeoPixel::Color(32, 32, 0);
uint32_t wild_color = Adafruit_NeoPixel::Color(200, 200, 0);
uint32_t bright_yellow = Adafruit_NeoPixel::Color(200, 200, 0);
uint32_t bright_green = Adafruit_NeoPixel::Color(0, 200, 0);
uint32_t bright_white = Adafruit_NeoPixel::Color(200, 200, 200);
uint32_t purple = Adafruit_NeoPixel::Color(172, 40, 146);

int color_array[0]
color_array[0] = Adafruit_NeoPixel::Color(0,0,0);
...
void setup(void)
{
    ...
    all_pixels(strip, off);
    all_pixels(ring, Adafruit_NeoPixel::Color(0, 0, 0));


    // Set up a rainbow pattern on the strip
    strip.setPixelColor(0, first_color);
    strip.setPixelColor(1, second_color);
...
void loop(void)
{
    ...
    {
        //rotate_down(strip);
        shift_down(strip, first_color);
        rotate_down(ring);
    }
    else if (sensorValue > 10)
    {
        //otate_up(strip);
        shift_up(strip, second_color);
        rotate_up(ring);
    }
